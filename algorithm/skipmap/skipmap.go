...
// 2. 创建新节点，并在目标位置插入节点
// 3. 调整跳表 backward 指针等
func (m *Skipmap) Set(key int, elem interface{}) *Node {
	var (
		// update 用于记录每层待更新的节点
...
// Del 用于删除跳表中指定的节点。
func (m *Skipmap) Del(key int) *Node {
	// 第一步，找到需要删除节点
	var (
		update     [MaxLevel]*Node
...
func (m *Skipmap) Get(key int) *Node {
	var (
		update     [MaxLevel]*Node
		targetNode = &Node{key: key}
	)
...
// 2. 采用更加昂贵的操作：删除再添加。
func (m *Skipmap) UpdateKey(curKey int, elem int64, newKey int) *Node {
	var (
		update     [MaxLevel]*Node
...
func (m *Skipmap) canUpdateKeyFor(node *Node, newKey int) bool {
	if (node.backward == nil || compareKey(node.backward.key, newKey) < 0) &&
		(node.level[0].forward == nil || compareKey(node.level[0].forward.key, newKey) > 0) {
		return true
	}
	return false
}
...
func (r *Range) GteMin(v int) bool {
	if r.ExcludeMin {
		return compareKey(v, r.Min) > 0
	}
	return compareKey(v, r.Min) >= 0
}

func (r *Range) LteMax(v int) bool {
	if r.ExcludeMax {
		return compareKey(v, r.Max) < 0
	}
	return compareKey(v, r.Min) <= 0
}
...
func (m *Skipmap) Rank(key int, elem int64) int {
	rank := 0
